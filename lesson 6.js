...
function editNumberOfPages(title, pages, books) {
    if (typeof title != String) return
    if (Array.isArray(books)) return
    var currentBook = books[index].title;
    var singleBook = books.find(function (currentBook) { return currentBook.title === title })

    if (singleBook) {
        singleBook.pages = pages
    }s
}
